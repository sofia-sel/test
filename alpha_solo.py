...
def generate():
    ...
    while True:
        ...
        # Convert thermal frame to grayscale to identify temperature values
        gray_thermal_frame = cv2.cvtColor(thermal_frame, cv2.COLOR_BGR2GRAY)

        # Create a mask where temperatures below 40°C are marked as transparent
        _, mask = cv2.threshold(gray_thermal_frame, int(40), 255, cv2.THRESH_BINARY)
        ...
        result_frame = cv2.add(hd_frame * mask, blended_frame * (1 - mask))
