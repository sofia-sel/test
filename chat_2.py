        ...
        while True:
            ...
            if key == ord("s"):  # If s is chosen, save an image to file
                ...
                if colormap_index == len(colormap_list):
                    colormap_index = 0
            elif key == ord("x"):  # If x is chosen cycle the colormap used back
                colormap_index -= 1
                if colormap_index < 0:
                    colormap_index = len(colormap_list) - 1
            elif key == ord("f"):  # If f is chosen toggle the image filtering
                filter_image = not filter_image
                print(f"Filter On: {filter_image}")
            elif key == ord("t"):  # If t is chosen toggle the units used for Temperature
                use_f = not use_f
            ...
            elif key == 27:  # Break if escape key is pressed
