...
def generate():
    ...
    while True:
        ...
        # Resize thermal frame to match HD frame dimensions
        thermal_frame_resized = cv2.resize(thermal_frame, (hd_frame.shape[1], hd_frame.shape[0]))

        # Overlay thermal frame on HD frame using alpha blending
        combined_frame = cv2.addWeighted(hd_frame, 1 - ALPHA, thermal_frame_resized, ALPHA, 0)
